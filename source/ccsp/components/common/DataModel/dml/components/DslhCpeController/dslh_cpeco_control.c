    ...
    )
{
    ...
    {
        AnscTraceWarning(("DBG-TBL-FLOW: RegisterObject start name=%s has{i}=%s paramArray=%p\n",
        pObjectDescr->Name ? pObjectDescr->Name : "NULL",
        (pObjectDescr->Name && _ansc_strstr(pObjectDescr->Name, "{i}")) ? "yes" : "no",
        hParamArray));
    ...
    while (pParamDescr != NULL && pParamDescr->Name != NULL )
    {
        ...
        if ( pMyObject->uParameterArrayIndex < pMyObject->uParameterCount )
        {
            if ( pParameterArray && pParameterHolder )
            {
                ...
                AnscTraceWarning(("DBG-TBL-FLOW: ns-add idx=%lu ns=%s obj=%s param=%s has_i=%s\n",
                ...
                (_ansc_strstr(pParameterArray[pMyObject->uParameterArrayIndex].name_space ? pParameterArray[pMyObject->uParameterArrayIndex].name_space : "", "{i}")) ? "yes" : "no"));
        ...
        {
            AnscTraceWarning(("DBG-TBL-FLOW: ERROR array overflow idx=%lu max=%lu obj=%s param=%s\n",
            pMyObject->uParameterArrayIndex, pMyObject->uParameterCount,
            pObjectDescr->Name ? pObjectDescr->Name : "NULL",
            pParamDescr->Name ? pParamDescr->Name : "NULL"));
    ...
    AnscTraceWarning(("DBG-TBL-FLOW: RegisterObject end name=%s paramCount=%lu currentIdx=%lu\n",
    pObjectDescr->Name ? pObjectDescr->Name : "NULL",
    ulParamCount,
    pMyObject->uParameterArrayIndex));
	...
	)
{
	...
	{   
		...
		if((!bHasChildParam) && (pObjEntity->ObjDescr))
		{
            ...
            pParameterArray[pMyObject->uParameterArrayIndex].name_space   = &pParameterHolder[pMyObject->uParameterIndex];
            AnscTraceWarning(("DBG-TBL-FLOW: ns-add-dummy idx=%lu ns=%s obj=%s has_i=%s\n",
            pMyObject->uParameterArrayIndex,
            pParameterArray[pMyObject->uParameterArrayIndex].name_space ? pParameterArray[pMyObject->uParameterArrayIndex].name_space : "NULL",
    ...
    )
{
    ...
    AnscTraceWarning(("DBG-TBL-FLOW: DslhCpecoRegisterDataModelInternal start comp=%s cr=%s xmlfile=%s\n",
    pCompName ? pCompName : "NULL",
    pCR_id ? pCR_id : "NULL",
    pXMLFile ? pXMLFile : "NULL"));
    ...
    if ( !pParameterArray || !pParameterHolder )
    {
        AnscTraceWarning(("DBG-TBL-FLOW: ERROR array/holder allocation failed pArray=%p pHolder=%p max_count=%d\n",
    ...
    */
    ...
    for (uIndex = 0; uIndex < uCount; uIndex += CCSP_MAX_PARAMETER_PER_REGISTRATION)
    {
        ...
        do
        {
            if ( uCount > 0 )
            {
            AnscTraceWarning(("DBG-COMP-REG: about to registerCapabilities comp=%s cr=%s count=%lu first=%s last=%s\n",
            pTmpCompName,
            pTmpCRName,
            uCount,
            ...
            {
            AnscTraceWarning(("DBG-COMP-REG: about to registerCapabilities comp=%s cr=%s count=0\n",
            pTmpCompName, pTmpCRName));
            }
            returnStatus = 
                CcspBaseIf_registerCapabilities
