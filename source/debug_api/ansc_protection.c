    ...
    )
{
    ...
    if( ! pFreedLock )
    {
        CcspTraceWarning(("!!! Spin Lock Profiling: memory ran out.\n"));
        return ANSC_STATUS_SUCCESS;
    }
    ...
    )
{
    ...
    if ( gbAnscSpinLockProfiling )
    {
        ...
        {
            CcspTraceError(("!!! Spin Lock total time overflow @ [%s]--<%d>\n", pFileName, iLineNo));
        }
    ...
    )
{
    ...
    CcspTraceError(("  No  Flags  Hits  Total  Avg   Cur   Max  InitName  InitLine  MaxName  MaxLine \n"));
