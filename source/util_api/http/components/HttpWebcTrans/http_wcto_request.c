    ...
    )
{
    ...
    pReqInfo = AnscAllocateMemory(sizeof(HTTP_REQUEST_INFO));
    if ( !pReqInfo )
    {
     ...
     */
    pHttpHfoUserAgent = AnscAllocateMemory(sizeof(HTTP_HFO_USER_AGENT));
    ...
    snprintf(pHttpHfoUserAgent->ProductToken, sizeof(pHttpHfoUserAgent->ProductToken), "%s", pSimpleClient->GetProductName((ANSC_HANDLE)pSimpleClient));

    returnStatus = pBmoReq->AddHeaderField((ANSC_HANDLE) pBmoReq, (ANSC_HANDLE) pHttpHfoUserAgent);
     ...
     * header field.
     */
    pHttpHfoHost = AnscAllocateMemory(sizeof(HTTP_HFO_HOST));
    if ( !pHttpHfoHost )
    {
        return ANSC_STATUS_RESOURCES;
    }
    ...
    if (strchr(pReqUri->HostName, ':'))
    {
        if (snprintf(pHttpHfoHost->HostName, sizeof(pHttpHfoHost->HostName), "[%s]", pReqUri->HostName) >= (int) sizeof(pHttpHfoHost->HostName))
        {
	    AnscFreeMemory(pHttpHfoHost);
            return ANSC_STATUS_RESOURCES;
        }
    }
    else
    ...
    returnStatus = pBmoReq->AddHeaderField((ANSC_HANDLE) pBmoReq, (ANSC_HANDLE) pHttpHfoHost);
    ...
    switch ( ulMethod )
    {
        case HTTP_METHOD_CODE_OPTIONS:
            returnStatus = pMyObject->DoOptions((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq);
            break;

        case HTTP_METHOD_CODE_GET:
            returnStatus = pMyObject->DoGet((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq);
            break;

        case HTTP_METHOD_CODE_HEAD:
            returnStatus = pMyObject->DoHead((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq);
            break;

        case HTTP_METHOD_CODE_POST:
            returnStatus = pMyObject->DoPost((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq);
            break;

        case HTTP_METHOD_CODE_PUT:
            returnStatus = pMyObject->DoPut((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq);
            break;

        case HTTP_METHOD_CODE_DELETE:
            returnStatus = pMyObject->DoDelete((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq);
            break;

        case HTTP_METHOD_CODE_TRACE:
            returnStatus = pMyObject->DoTrace((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq);
            break;

        case HTTP_METHOD_CODE_CONNECT:
            returnStatus = pMyObject->DoConnect((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq);
            break;
    ...
    returnStatus = pBspIf->Polish(pBspIf->hOwnerContext, (ANSC_HANDLE) pBmoReq, pMyObject->hBspReqContext);
    ...
    returnStatus = pCasIf->ProcessRequest(pCasIf->hOwnerContext, (ANSC_HANDLE) pBmoReq);

    if ( returnStatus == ANSC_STATUS_SUCCESS )
    {
        returnStatus = pMyObject->Send((ANSC_HANDLE) pMyObject, (ANSC_HANDLE) pBmoReq, HTTP_WCTO_SFLAG_HEADERS | HTTP_WCTO_SFLAG_BODY);
