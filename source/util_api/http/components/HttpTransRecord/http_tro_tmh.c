    ...
    )
{
    ...
    if ( pMyObject->TransState == HTTP_TRO_STATE_FINISHED )
    {
    ...
    switch ( ulEvent )
    {
        case    HTTP_TMH_EVENT_ARRIVED_HEADER :
                 ...
                 */
                if ( (pMyObject->TransState == HTTP_TRO_STATE_INITIALIZED     ) ||
                     (pMyObject->TransState == HTTP_TRO_STATE_CLIENT_CONNECTED) )
                {
                    returnStatus =
                    ...
                    else if ( pMyObject->TransState == HTTP_TRO_STATE_FINISHED )
                    {
                        return  returnStatus;
                    }
                    else
                ...
                /*
                 * It has been proved that the idea of "Persistent Connection" may cause a lot of
                 * problems for HTTP proxies. One of the examples is the fact that Microsoft's IE
                 * browser always sends "Proxy-Connection: Keep-Alive" header in every request to
                 * the explicitly-configured proxy along with HTTP/1.0 version number, but it will
                 * still recognize the "Connection: close" header as defined in HTTP/1.1, and acts
                 * as an HTTP/1.1 client upon receiving this header.
                 */
                if ( pBmoReq->IsProxyKeepAlive((ANSC_HANDLE)pBmoReq) )
                {
        ...
        case    HTTP_TMH_EVENT_COMPLETED :
                 ...
                 * a new message object will be created by the tro_query() function next time.
                 */
    ...
    )
{
    ...
    switch ( ulEvent )
    {
        case    HTTP_TMH_EVENT_ARRIVED_HEADER :
                 ...
                 * as an HTTP/1.1 client upon receiving this header.
                 */
                if ( pBmoRep->IsCloseConnection((ANSC_HANDLE)pBmoRep) )
                {
                    pMyObject->bCloseConnection = TRUE;
                }
