...
{
    RBUS_LOG_ERR("DBG-TBL-FLOW: AddTblRow request dst=%s obj=%s session=%d\n",
    ...
    if((rc = rbusTable_addRow(bus_info->rbus_handle, objectName, NULL, (unsigned int *)instanceNumber)) != RBUS_ERROR_SUCCESS)
    {
        ...
        RBUS_LOG_ERR("DBG-INST: ERROR AddTblRow allocation failed obj=%s rc=%d\n", objectName, rc);
        RBUS_LOG_ERR("AddTblRow for %s failed with Error: %d\n", dst_component_id, ret);
    }
    else
    {
        RBUS_LOG_ERR("DBG-INST: allocated instance=%u obj=%s\n", *instanceNumber, objectName);
    }
    return ret;
}
...
{
    RBUS_LOG_ERR("DBG-TBL-FLOW: DeleteTblRow request dst=%s obj=%s session=%d\n",
    dst_component_id ? dst_component_id : "NULL",
    objectName ? objectName : "NULL",
    sessionId);
